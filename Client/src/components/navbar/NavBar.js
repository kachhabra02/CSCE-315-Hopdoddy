...
function NavBar({onUpdate}) {
    ...
    useLayoutEffect(() => {
        ...
        if (el.length > 0) {
            // console.log(el)
            el.html(`<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
    ...
    // useLayoutEffect(() => {
    //     // sleep(1)
    //     // let el = gtref.current.querySelector("a");
    //     let el = document.querySelector("#google_translate_element a")
    //     console.log(gtref.current)

    //     if (el && !isCustomized?.success) {
    //         console.log(el)
    ...
    //         tryCustomizing({success: true})
    //         console.log("success")
    //     } else if (!isCustomized?.success) {
    //         // console.log("updating...")
    //         setTimeout(() => tryCustomizing({}), 500)
    //     }
    // }, [isCustomized])
    ...
    return (
        <Toolbar id = 'navbar'>
            ...
            <Box sx={{ mx: 2 }}>
                <div id="google_translate_element" ref={gtref}></div>
            </Box>
        </Toolbar>
    )
}
